  ...
  return (
    <div className="dropdown-indicator-with-add">
      ...
      <button
        ...
        onClick={(e) => {
          e.stopPropagation();
          if (selectProps && typeof selectProps.onAddNew === "function") {
            selectProps.onAddNew();
          }
        }}
...
export default function AddParticipantForm({
  ...
  useEffect(() => {
    ...
    async function loadAgents() {
      ...
      try {
        ...
        const opts = (raw || []).map((a) => {
          const agentNumber = a.AgentNumber ?? a.AgentNo ?? "";
          const agentName = a.FinancialAdvisor ?? a.FinancialAdvisorName ?? a.AgentName ?? "";
          return {
            value: agentNumber || null,
            label: `${agentNumber} - ${agentName}`,
            raw: a,
  ...
  function isAnyFormFieldFilled() {
    const fields = {
      participantName, firstName, lastName, phoneNumber,
  ...
  function validateBeforeConfirm() {
    if (!participantName || !firstName || !lastName || !phoneNumber || 
        !address1 || !city || !stateValue || !emailAddress) {
      setErrorMessage("Please fill all mandatory fields correctly.");
      return false;
    }
    ...
    if (country?.toUpperCase() === "USA" && postalCode && postalCode.length !== 5) {
      setErrorMessage("For USA, Postal Code must be 5 digits.");
      return false;
    }
  ...
  function buildParticipantPayload() {
    return {
      ...
      LastName: lastName || null,
      ParticipantType: participantType || null,
      ParticipantFinancialAdvisor: participantFinancialAdvisor?.value || null,
      ParticipantFinancialAdvisorName: participantFinancialAdvisor?.label || null,
      OrganizationName: organizationName || null,
      IsActive: isActive || "Y",
      OpenDate: formatDateForApi(openDate),
      EmailAddress: emailAddress || null,
  ...
  async function handleAddClick() {
    ...
    try {
      ...
      console.error("[AddParticipantForm] handleAddClick", err);
      setApiError(err?.message || "Failed to add participant.");
      setSaving(false);
      setShowConfirm(false);
    }
  }
  ...
  async function handleAddNewAdvisor() {
    if (typeof onRequestAddUser === "function") {
      try {
        onRequestAddUser("advisor");
        sessionStorage.setItem("preselect_add_user_role", "advisor");
      } catch (err) {
        console.error("onRequestAddUser threw:", err);
      }
      return;
          ...
          <div className="form-grid">
            <div className="personal-details">
              ...
              <div className="field-row">
                <label className="field suffix-field">
                  <span className="field-label">Suffix</span>
                  <select value={suffix} onChange={(e) => setSuffix(e.target.value)}>
                ...
                <label className="field ParticipantName">
                  <span className="field-label">Participant Name *</span>
                  <input 
                    type="text" 
              ...
              <div className="field-row">
                <label className="field">
                  ...
                  <input 
                    type="text" 
                    placeholder="Donor / Investor / Other" 
              ...
              <div className="field-row">
                ...
                <label className="field state">
                  ...
                  <span className="field-label">
                    Zipcode {country?.toUpperCase() === "USA" ? "*" : ""}
                  </span>
                  <input 
                    type="text" 
                    value={postalCode} 
            ...
            <div className="sidebar-section">
              <div>
                <h4 className="Participantlabel">Participation</h4>
                <div className="field-row advisor-field-row">
                  <label className="field participant-field">
                    ...
                    <Select
                      ...
                      components={{ DropdownIndicator: DropdownIndicatorWithAdd }}
                      onAddNew={handleAddNewAdvisor}
                      noOptionsMessage={() => loadingAdvisors ? "Loading..." : "No advisors found"}
                    ...
                    valueContainer: (provided) => ({ ...provided, padding: "0 6px", margin: 0 }),
                      ...
                      formatOptionLabel={(option) => (
                        <div className="advisor-option-label">{option.label}</div>
                      )}
                      classNamePrefix="participant"
                    />
                  </label>
                </div>
              </div>
      ...
      {showConfirm && (
        <div className="confirmation-modal">
          <div className="confirmation-card">
            <h2>Confirmation</h2>
            <ul>
              {Object.entries(buildParticipantPayload()).map(([key, value]) => {
                if (value === null || value === "") return null;
                return (
                  <li key={key}>
